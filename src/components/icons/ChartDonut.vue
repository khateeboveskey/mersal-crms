<template>
    <div>
        <svg :class="`w-${setting.tailwindSize} h-${setting.tailwindSize}`" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            <path stroke="none" d="M0 0h24v24H0z" />
            <path d="M10 3.2a9 9 0 1 0 10.8 10.8a1 1 0 0 0 -1 -1h-3.8a4.1 4.1 0 1 1 -5 -5v-4a.9 .9 0 0 0 -1 -.8" />
            <path d="M15 3.5a9 9 0 0 1 5.5 5.5h-4.5a9 9 0 0 0 -1 -1v-4.5" />
        </svg>
    </div>
</template>

<script setup>
import { useIconsSettingStore } from '@/stores/useIconsSettingStore';
const setting = useIconsSettingStore();
</script>